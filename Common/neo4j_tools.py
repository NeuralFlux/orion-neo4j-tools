...
class Neo4jTools:
    ...
    def __init__(
        ...
        self.password = "12345678" if password is None else password
        self.graph_db_uri = f"bolt://{neo4j_host}:{bolt_port}"
        self.graph_db_auth = ("neo4j", self.password)
        self.neo4j_driver = neo4j.GraphDatabase.driver(
            self.graph_db_uri, auth=self.graph_db_auth
        )
    ...
    def import_csv_files(
        ...
        import_results: subprocess.CompletedProcess = subprocess.run(
            neo4j_import_cmd, cwd=graph_directory, capture_output=True
        )
        ...
        if import_results_return_code != 0:
            error_message = (
                f"Neo4j import subprocess error (ExitCode {import_results_return_code}): "
                f'{import_results.stderr.decode("UTF-8")}'
            )
            self.logger.error(error_message)
        return import_results_return_code

    def load_backup_dump(self, dump_file_path: str = None):
        password_exit_code = self.set_initial_password()
        if password_exit_code != 0:
            return password_exit_code
        ...
        load_results: subprocess.CompletedProcess = subprocess.run(
            neo4j_load_cmd, capture_output=True
        )
        self.logger.info(load_results.stdout)
        load_results_return_code = load_results.returncode
        if load_results_return_code != 0:
            error_message = (
                f"Neo4j load subprocess error (ExitCode {load_results_return_code}): "
    ...
    def migrate_dump_to_neo4j_5(self):
        ...
        migrate_results: subprocess.CompletedProcess = subprocess.run(
            neo4j_migrate_cmd, capture_output=True
        )
        self.logger.info(migrate_results.stdout)
        results_return_code = migrate_results.returncode
        if results_return_code != 0:
            error_message = (
                f"Neo4j migrate subprocess error (ExitCode {results_return_code}): "
                f'{migrate_results.stderr.decode("UTF-8")}'
            )
            self.logger.error(error_message)
        return results_return_code

    def create_backup_dump(self, dump_directory: str = None):
        ...
        dump_results: subprocess.CompletedProcess = subprocess.run(
            neo4j_dump_cmd, capture_output=True
        )
        self.logger.info(dump_results.stdout)
        dump_results_return_code = dump_results.returncode
        if dump_results_return_code != 0:
            error_message = (
                f"Neo4j dump subprocess error (ExitCode {dump_results_return_code}): "
    ...
    def __issue_neo4j_command(self, command: str):
        ...
        neo4j_results: subprocess.CompletedProcess = subprocess.run(
            neo4j_cmd, capture_output=True
        )
        self.logger.info(neo4j_results.stdout)
        neo4j_results_return_code = neo4j_results.returncode
        if neo4j_results_return_code != 0:
            error_message = (
                f"Neo4j {command} subprocess error (ExitCode {neo4j_results_return_code}): "
    ...
    def set_initial_password(self):
        ...
        if neo4j_results_return_code != 0:
            error_message = (
                f"Neo4j {neo4j_cmd} subprocess error (ExitCode {neo4j_results_return_code}): "
                f'{neo4j_results.stderr.decode("UTF-8")}'
            )
            self.logger.error(error_message)
        return neo4j_results_return_code
    ...
    def add_db_indexes(self):
        ...
        try:
            with self.neo4j_driver.session() as session:
                ...
                # node name index
                node_name_index_cypher = (
                    f"CREATE INDEX node_name_index FOR (n:`{NAMED_THING}`) ON (n.name)"
                )
                ...
                self.logger.info(
                    f"Adding node id indexes for node labels: {node_labels}"
                )
                for node_label in node_labels:
                    node_label_index = f'node_id_{node_label.replace(":", "_")}'
                    node_name_index_cypher = (
                        f"CREATE CONSTRAINT {node_label_index} FOR (n:`{node_label}`) "
                ...
                for result in retrieve_indexes_results:
                    if result["name"] in index_names:
                        if result["state"] == "ONLINE":
                            ...
                            self.logger.error(
                                f"Oh No. Index {result['name']} has state {result['state']} "
                                f"but should be online."
                            )
                if indexes_added != confirmed_index_count:
                    self.logger.error(
                        f"Oh No. Tried to add {indexes_added} indexes "
    ...
    def wait_for_neo4j_initialization(self, counter: int = 1, max_retries: int = 10):
        try:
            with self.neo4j_driver.session() as session:
                ...
                self.logger.info(
                    f"Neo4j ready at {self.host}:{self.http_port}, {self.graph_db_uri}."
                )
                return 0
            ...
            if counter > max_retries:
                self.logger.error(
                    f"Waited too long for Neo4j initialization... giving up.."
                )
                return 1
            self.logger.info(
                f"Waiting for Neo4j container to finish initialization... {repr(e)}"
            )
            time.sleep(10)
...
def create_neo4j_dump(
    ...
    if os.path.exists(csv_nodes_file_path) and os.path.exists(csv_edges_file_path):
        if logger:
            logger.info(
                f"CSV files were already created for {graph_id}({graph_version})"
            )
    else:
        if logger:
    ...
    try:
        import_exit_code = neo4j_access.import_csv_files(
    ...
    if logger:
        logger.info(
            f"Success! Neo4j dump created with indexes for {graph_id}({graph_version})"
        )
    return True
